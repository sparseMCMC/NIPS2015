...
DIR_RESULTS = "RESULTS_JOINT_ZOPTIM/"
...
for(OPTION_ARD in c("False")) {

print("Plotting PSRF")
pdf(paste(DIR_RESULTS, "PLOT_convergence_coal_nz", OPTION_NZ, "_ard", OPTION_ARD, ".pdf", sep=""))
...
for(OPTION_SAMPLER in c(1:2)) {
  ...
  for(OPTION_SEED in c(0:(N_SEEDS-1))) {
    ...
    aa = as.matrix(read.table(filename_samples, colClasses="numeric"))
    ...
    bb = mean(as.matrix(read.table(filename_times, colClasses="numeric")))
    ...
    cc = as.matrix(read.table(filename_accepts, colClasses="numeric"))
...
plot(psrf[[1]][[1]], psrf[[1]][[2]][,1], type="l", ylim=c(1,4), xlim=range(psrf[[1]][[1]]), xlab="Iteration", ylab="PSRF", main=paste("Coal dataset -", OPTION_NZ, "inducing points"), col=rgb(0,0,1,0.3))
...
write.table(cbind(psrf[[1]][[1]], psrf[[1]][[2]]), file=paste("coal_HMC_ard", OPTION_ARD, ".txt", sep=""), quote=FALSE, col.names=F, row.names=F)
write.table(cbind(psrf[[2]][[1]], psrf[[2]][[2]]), file=paste("coal_Gibbs_ard", OPTION_ARD, ".txt", sep=""), quote=FALSE, col.names=F, row.names=F)
