...
print("Plotting PSRF")
pdf(paste(DIR_RESULTS, "PLOT_convergence_image_nz", OPTION_NZ, "_ard", OPTION_ARD, ".pdf", sep=""))
...
plot(psrf[[1]][[1]], psrf[[1]][[2]][,1], type="l", ylim=c(1,4), xlim=range(psrf[[1]][[1]]), xlab="Iteration", ylab="PSRF", main=paste("Image dataset -", c("RBF", "ARD")[(OPTION_ARD=="True")+1], "-", OPTION_NZ, "inducing points"), col=rgb(0,0,1,0.3))
...
write.table(cbind(psrf[[1]][[1]], psrf[[1]][[2]]), file=paste("image_HMC_ard", OPTION_ARD, ".txt", sep=""), quote=FALSE, col.names=F, row.names=F)
write.table(cbind(psrf[[2]][[1]], psrf[[2]][[2]]), file=paste("image_Gibbs_ard", OPTION_ARD, ".txt", sep=""), quote=FALSE, col.names=F, row.names=F)
