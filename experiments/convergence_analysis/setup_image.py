...
## ********** Load image dataset
def load_data():
    X = np.genfromtxt('../../image_xtrain.csv', delimiter=',')
    Y = np.genfromtxt('../../image_ytrain.csv', delimiter=',')
    Y.resize(Y.size,1)
    
    X_test = np.genfromtxt('../../image_xtest.csv', delimiter=',')
    Y_test = np.genfromtxt('../../image_ytest.csv', delimiter=',')
    Y_test.resize(Y_test.size,1)
    
    print "Image dataset loaded\n"
    return Dataset(X, Y, X_test, Y_test, X.shape[0], X.shape[1])
## ******************************
...
def kernel_priors(kernel):
    ...
    kernel.rbf.variance.set_prior(GPy.priors.Gamma(1.1,1.0), warning=False)
    kernel.rbf.lengthscale.set_prior(GPy.priors.Gamma(1.1,1.0), warning=False)
...
def setup_kernel(dataset, ARD):
    kernel = GPy.kern.RBF(input_dim=dataset.xtrain.shape[1], ARD=ARD) + GPy.kern.White(1, variance=1e-6)
...
def optimise_GP_classifier(dataset, Z, likelihood, kernel, OPTION_ARD):
    ...
    m_approx.Z.unfix()
    m_approx.optimize('bfgs', max_iters=10000, messages=1)
